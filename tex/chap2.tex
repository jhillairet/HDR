...
\begin{subequations}
	...
	\end{eqnarray}
\end{subequations}
where $n_s$ is the density, $m_s$ the mass, $e$ the (signed) elementary charge and $Z$ the atomic number (number of protons) for the species $s$ (electron or ions). 
...
\end{equation}
with\marginnote[+2cm]{Note that $S^{2}-D^{2}=RL$.}:
\begin{subequations}
	\begin{eqnarray}
		A & = & S\\
		B & = & (S - n_\parallel^2)(S + P) - D^2 \\
...
\begin{subequations}
	\begin{eqnarray}
		S &\approx& \frac{\omega_{i}^2}{\Omega_{ci}^2 \left(1 - \omega^2/\Omega_{i}^2 \right)} \ll -1 \\
...
The roots of the dispersion equation Eqs.(\ref{eq:nperp_solution_general}) are:
\begin{subequations}
\begin{eqnarray}
n_{\perp, F}^2 &\approx& \frac{\left(R - n_\parallel^2 \right)\left(R - n_\parallel^2\right)}{S-n_\parallel^2} \\
			   &\approx& \left(S-n_\parallel^2 \right) - \frac{D^2}{S-n_\parallel^2} \nonumber \\
n_{\perp, S}^2 &\approx& P\left(1 - \frac{n_\parallel^2}{S}\right)			  
\end{eqnarray}
\end{subequations}
where the subscript $_F$ and $_S$ stands for \textit{Fast} and \textit{Slow} wave modes. 
...
% ###################################################
% ###################################################
\subsubsection{Coupling Resistance}
\todo{reformuler}
...
aspects theorique couplage\sidecite{messiaen1982}

Works well with experiments\sidecite{koch1988}
...
% ###################################################
% ###################################################
\subsubsection{Wave propagation and absorption in the plasma}
