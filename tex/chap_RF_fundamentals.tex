...
\marginnote[*-2]{Parts of this Section are taken from the documentation of the open-source Python package \href{http://scikit-rf.org/}{scikit-rf} written by the author. More details on the \href{http://scikit-rf.org/}{scikit-rf} package is given in Section~\ref{sec:OSS}.}
...
\marginnote{\textit{Return loss} RL is a measure of the effectiveness of power delivery from a transmission line to a load, such as an antenna (\citeauthyear{bird2009}). Expressed in dB, return loss is defined as:
$$
\mathrm{RL} = 10 \log_{10} \left( \frac{P_i}{P_r} \right)
$$
Hence it is the negative of the magnitude of the reflection coefficient in \si{dB}. Since power is proportional to the square of the voltage, return loss is given by 
...
\marginnote{The total loss $\mathrm{TL}$ in a line in \si{dB} can also be stated as:
$$
...
\end{equation}
with $\sigma=1/\rho$ the metal conductivity (in $[\si{S/m}]$) and $\rho$ the metal resistivity (in $[\si{\Omega m}]$). 
